...
internal class CarouselView(context: Context, attrs: AttributeSet? = null) : FrameLayout(context, attrs) {
	...
	fun scroll(newText: String) {
		...
		bottomView?.apply {
			...
			animate().apply {
				...
				translationY(0f)
				withStartAction {
					Timber.d("start bottom animation")
				}
			}
		}
